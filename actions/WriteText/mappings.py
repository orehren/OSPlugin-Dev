...
class KeyMapper:
    ...
    def _setup_xkb(self):
        try:
            ...
            log.debug(f"Attributes of names: {dir(xkb)}")
            log.debug(f"Attributes of names: {dir(xkb.layout_index_get)}")
            ...
            self.xkb_keymap = self.xkb_context.keymap_from_string(keymap_str, xkb.XKB_COMPOSE_FORMAT_TEXT_V1, 1)
